...
class CoQAGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i in range(start_index, end_index):
            ...
            # a, b, c = pd.isnull(np.array(context_inputs['context'])).any(), pd.isnull(np.array(context_inputs['context_pos'])).any(), pd.isnull(np.array(context_inputs['context_ent'])).any()
            # try:
            #     assert a == b == c == False
            # except:
            #     print(a, b, c)
